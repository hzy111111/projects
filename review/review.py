...
def get_features_by_wordbag(pos_text_train, neg_text_train, pos_text_test, neg_text_test):
    ...
    neg_text_test，测试负样本标签，list
    :return: 特征向量化后
    x_train, 训练样本内容，array，(n_samples, max_words)
    y_train, 训练样本标签，array，(n_samples, )
    x_test, 测试样本内容，array，(n_samples, max_words)
...
def cnn():
    ...
    :return: 返回cnn模型
    """
    input_dim = max_words#500
    input_length = output_dim#100
...
def get_features(load_type):
    ...
    if load_type == 0:
        x_train, y_train, x_test, y_test = get_features_by_wordbag(pos_text_train, neg_text_train,
                                                                   pos_text_test, neg_text_test)
        return x_train, y_train, x_test, y_test
    if load_type == 1:
        x_train, y_train, x_test, y_test = get_features_by_vocabulary(pos_text_train, neg_text_train,
                                                                      pos_text_test, neg_text_test)
        return x_train, y_train, x_test, y_test
    if load_type == 2:
...
if __name__ == "__main__":
    ...
    load_type = 1 #0:wordbag 1:vocabulary 2:word2Vec 3:doc2Vec
